...
function DisplayEditDesktopBIForm(item, index, addHeaderButtons) {
    ...
    bucketEditDisplay = bucketEditDisplay + tblRowPrefix + tblCellPrefix + "" + tblCellSuffix + tblRowSuffix;                  //empty row 
    bucketEditDisplay = bucketEditDisplay + tblCellPrefix + "Latitude: " + data[5] + tblCellSuffix + tblRowSuffix;             //Latitude
    bucketEditDisplay = bucketEditDisplay + tblRowPrefix + tblCellPrefix + "" + tblCellSuffix + tblRowSuffix;                  //empty row 
    bucketEditDisplay = bucketEditDisplay + tblCellPrefix + "Longitude: " + data[6] + tblCellSuffix + tblRowSuffix;            //Longitude
...
function DisplayEditBIForm(item) {
    ...
    $("#BILatitude").val(data[5]);
    $("#BILongitude").val(data[6]);

    $("#BucketListRecordId").val(data[7]); //TODO - make this dynamic so the record id is the last element
            ...
            if (innerCtr == 1) {
                ...
                if (currentFilter == 0) {
                    bucketList = bucketList + "<td class=\"DesktopTableCell\" onmouseover=\"LinkCursor(this)\" onmouseout=\"NormalCursor(this)\" id=\"blLink" + index + "\" name=\"blLink" + index + "\">" +
                              "<a id=\"blAHrefLink" + index + "\" onClick=\"ProcessEdit(" + row[7] + "); return false;\"  >" + (index + 1) + " - "
                                  + row[innerCtr]
                    ...
                    bucketList = bucketList + "<td class=\"DesktopTableCell\" onmouseover=\"LinkCursor(this)\" onmouseout=\"NormalCursor(this)\" id=\"blLink" + index + "\" name=\"blLink" + index + "\">" +
                        "<a onClick=\"ProcessEdit(" + row[7] + "); return false;\"  >" + (index + 1) + " - " + row[innerCtr];
